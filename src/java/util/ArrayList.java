...
/**
 * ArrayList是实现了List接口可调整大小的数组。实现了所有可选的List操作，
 * 并允许所有类型元素，包括null元素。除了实现List接口之外，
 * 该类还提供了一些方法来操作用于存储List的内部数组的大小。
 ...
 *
 * ArrayList的size、isEmpty、get、set、iterator和listIterator操作在常量时间内运行。
 * add操作在平摊常数时间内运行，也就是说，添加n个元素需要O(n)时间。
 * 所有其他操作都在线性时间内运行(粗略地说)。与LinkedList实现相比，常量因素要低。
 * 
 ...
 * 除了添加一个元素具有恒定的平摊时间代价之外，没有指定增长策略的细节。
 *
        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
     ...
     * first element is added(用于默认大小空实例的共享空数组实例，
     ...
     *
     * @param   minCapacity   the desired minimum capacity(//如果需要的最小容量大于最小的扩容数，则使用需要的最小容量扩容)
     */
